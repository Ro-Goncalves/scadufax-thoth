...
## 5. Busca Vetorial e Indexação

* **Milvus e FAISS:** Bibliotecas/bancos de dados especializados em busca de vizinhos mais próximos em espaços vetoriais de alta dimensão. O FAISS (Facebook AI Similarity Search) é uma biblioteca C++ amplamente usada em pesquisa; o Milvus é um banco de dados vetorial completo construído sobre ele. O Tesseract implementa o mesmo princípio de indexação (IVF + quantização int8) de forma customizada, sem depender dessas bibliotecas externas, para controle total sobre memória e latência.
* **Inverted File Index (IVF):** Técnica de indexação que organiza os vetores em grupos (*clusters*) durante o *build*. Na hora da busca, em vez de comparar a query com todos os vetores, o IVF calcula a distância da query até cada centróide e visita apenas os `nprobe` grupos mais próximos. O nome "invertido" vem da analogia com índices invertidos de texto: assim como um índice de livro mapeia palavra → páginas, o IVF mapeia centróide → lista de vetores pertencentes àquele cluster.
* **K-Means:** Algoritmo de agrupamento (*clustering*) não supervisionado. Recebe um conjunto de vetores e um número `K` e os divide em `K` grupos (clusters) de forma que vetores parecidos fiquem no mesmo grupo e vetores diferentes em grupos diferentes. Cada grupo é representado pelo seu **centróide** — a média aritmética de todos os vetores atribuídos a ele. No Tesseract, o K-Means roda uma única vez no *build* para organizar os vetores de referência antes de gerar o artefato V2.
* **Algoritmo de Lloyd:** A implementação mais comum do K-Means, que resolve o problema iterando dois passos até convergir: (1) **Atribuição** — cada vetor é atribuído ao centróide mais próximo; (2) **Atualização** — cada centróide é recalculado como a média dos vetores atribuídos a ele. O processo termina quando nenhum vetor muda de cluster (convergência) ou ao atingir o número máximo de iterações. Apesar da simplicidade, converge rapidamente em espaços de baixa dimensão como o nosso (14 dimensões).
